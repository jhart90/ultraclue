...
export function EnvelopeArt({ idPrefix = 'env' }: { idPrefix?: string }) {
  ...
  return (
    <>
      ...
      {/* envelope body */}
      <rect x="18" y="60" width="604" height="340" rx="12" fill={url('paper')} stroke="#b9a578" strokeWidth="2" />
      {/* grain overlay, clipped to the body */}
      <rect x="18" y="60" width="604" height="340" rx="12" filter={url('grain')} opacity="0.5" clipPath={url('clip')} />
      ...
      <path d="M40 66 L320 268 L600 66" fill="none" stroke="rgba(70,52,22,0.4)" strokeWidth="8" filter={url('soft')} />
      ...
      <g>
        <ellipse cx="320" cy="256" rx="42" ry="40" fill="rgba(0,0,0,0.28)" filter={url('soft')} />
        <circle cx="320" cy="250" r="40" fill={url('wax')} stroke="#5e0f18" strokeWidth="1.5" />
        <circle cx="320" cy="250" r="33" fill="none" stroke="rgba(255,255,255,0.18)" strokeWidth="1.5" />
        <path d="M320 230 L334 250 L320 270 L306 250 Z" fill="rgba(60,8,14,0.55)" stroke="rgba(255,200,200,0.18)" strokeWidth="1" />
      </g>
    </>
  );
}
