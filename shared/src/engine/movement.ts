import { BOARD, coordKey, type Board, type BoardCell, type Coord } from '../data/board';

// Movement runs over a "collapsed" graph where every room is a single node (a room counts as one
// space and ends your move when you enter it) and every corridor cell is its own node. Inter-floor
// stairs are included so the dice can route between sections; the 8 secret shortcuts are NOT part
// of dice movement (they are a separate mechanic, rendered only for now).
...
function nodeOf(cellMap: Map<string, BoardCell>, tile: Coord): string {
  ...
  return c && c.type === 'room' && c.roomId ? roomNode(c.roomId) : coordKey(tile);
}
...
function bfs(board: Board, start: Coord, steps: number, blocked: Set<string>): { graph: Map<string, string[]>; cellMap: Map<string, BoardCell>; startNode: string } & Bfs {
  ...
  while (queue.length) {
    ...
    // A room (other than the one you started in) ends your move — don't expand through it.
    if (isRoomNode(cur) && cur !== startNode) continue;
    if (d >= steps) continue;
    for (const n of graph.get(cur) ?? []) {
      if (!isRoomNode(n) && blocked.has(n)) continue; // corridor occupied by another piece
      if (!dist.has(n)) {
        dist.set(n, d + 1);
        parent.set(n, cur);
        queue.push(n);
